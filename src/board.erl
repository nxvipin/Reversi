...
find_direction_move(From, Direction, Board) ->
	find_direction_move(From, Direction, Board, {{count,0},
												 {start, none},
												 {path, []}
												}).
...
apply_moves([], Color, Board) ->
	Board;
apply_moves([H|T], Color, Board) ->
	{X, Y} = H,
	apply_moves(T, board:set(X,Y, Color, Board)).

play({{move, Move}, Color, Flips}, Board) ->
	All = Flips ++ [Move],
	apply_moves(All, Color, Board).


really_dumb_ai_play(Color, Board) ->
	apply_moves(lists:nth(1,find_valid_moves_for(Color,Board)), Color, Board).


rand_function(X, Y, Color, Board) ->
	io:format("Color ~p ~p ~p~n",[X, Y, Color]).
